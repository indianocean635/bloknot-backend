...
/**
 * Upload file to Selectel S3 with private ACL
 * @param {Buffer} buffer - File buffer
 * @param {string} fileName - File name
 * @param {string} contentType - File content type
 * @returns {Promise<string>} - File URL
 */
async function uploadPrivateFile(buffer, fileName, contentType = 'application/octet-stream') {
  try {
    const command = new PutObjectCommand({
      Bucket: process.env.S3_BUCKET,
      Key: fileName,
    ...
    const fileUrl = `https://s3.storage.selcloud.ru/${process.env.S3_BUCKET}/${fileName}`;
    console.log('Private file uploaded to S3:', fileUrl);
    return fileUrl;
  } catch (error) {
    console.error('S3 private upload error:', error);
