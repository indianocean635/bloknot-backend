...
// Регистрация пользователя (через magic link)
async function registerUser(userData) {
  console.log(`\n📝 РЕГИСТРАЦИЯ ${userData.email}`);
  
  try {
    // Используем request-login для регистрации
    const response = await axios.post(`${BASE_URL}/api/auth/request-login`, {
...
async function loginUser(userData) {
  ...
  try {
    ...
    console.log(`✅ Логин успешен: ${userData.email}`);
    return { user: response.data.user, cookies: cookieString };

  } catch (error) {
    console.error(`❌ Ошибка логина ${userData.email}:`, error.message);
    throw error;
  }
}
...
// Основной тест
async function runIsolationFullTest() {
  console.log('🚀 ЗАПУСКАЕМ ПОЛНЫЙ ТЕСТ ИЗОЛЯЦИИ ДАННЫХ');
  console.log('============================================================');
  ...
  try {
    ...
    // ШАГ 3: USER_A создает бизнес и специалиста
    console.log('\n📋 ШАГ 3: USER_A СОЗДАЕТ БИЗНЕС И СПЕЦИАЛИСТА');
    console.log('============================================');
    
    const businessA = await createBusiness(loginA.cookies, {
      name: 'Business A',
    ...
    const specialistA = await createSpecialist(loginA.cookies, {
      name: 'A_MASTER',
      email: 'master-a@example.com',
      phone: '+70000000004'
    });

    // ШАГ 4: USER_B НЕ создает бизнес
    console.log('\n📋 ШАГ 4: USER_B НЕ СОЗДАЕТ БИЗНЕС');
    console.log('============================================');
    console.log('USER_B будет тестироваться без бизнеса');

    // ШАГ 5: ПРОВЕРКА ИЗОЛЯЦИИ
    console.log('\n📋 ШАГ 5: ПРОВЕРКА ИЗОЛЯЦИИ ДАННЫХ');
    console.log('============================================');
    ...
    if (specialistsB.length !== 0) {
      ...
      console.error('❌ USER_B видит данные USER_A!');
    ...
    // ОЖИДАНИЕ: содержит "A_MASTER"
    const hasAMaster = specialistsA.some(s => s.name === 'A_MASTER');
    if (!hasAMaster) {
      console.error('\n🔥 ОШИБКА ТЕСТА!');
      console.error('❌ USER_A не видит своего специалиста A_MASTER');
      console.error('❌ СПЕЦИАЛИСТЫ USER_A:', specialistsA);
      process.exit(1);
    }
    
    console.log('✅ USER_A видит своего специалиста A_MASTER');

    // ШАГ 6: ФИНАЛЬНАЯ ПРОВЕРКА
    console.log('\n📋 ШАГ 6: ФИНАЛЬНАЯ ПРОВЕРКА ИЗОЛЯЦИИ');
    console.log('============================================');
    ...
    if (loginA.user.businessId === loginB.user.businessId) {
      throw new Error('🔥 КРИТИЧЕСКАЯ ОШИБКА: Пользователи имеют одинаковый businessId!');
    }
    console.log('✅ Пользователи имеют разные businessId');

    // Проверяем что USER_B не имеет бизнеса
    if (loginB.user.businessId !== null && loginB.user.businessId !== undefined) {
      console.log('⚠️  USER_B имеет businessId:', loginB.user.businessId);
    } else {
...
// Запуск теста
runIsolationFullTest().catch(error => {
